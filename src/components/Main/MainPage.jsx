...
const MainPage = () => {
    ...
    const onFinish = (values) => {
        if (values?.range?.from >= 0 && values?.range?.to >= 0) {
            let fi = `price>=${values?.range?.from}&price<=${values?.range?.to}`;
            if (values?.category?.length >= 0) {
                const filterCategory = values?.category.join(",");
                fi += `&category=${filterCategory}`;
            }
            setFilter(fi);
        }
    }
    const handleChangeFilter = (changedValues, values) => {
        ...
        if (changedValues.category) {
            ...
            if (cate && cate.length > 0) {
                const filterCategory = cate.join(",");
                            ...
                            <Col md={5} sm={0} xs={0}>
                                <div style={{ padding: "20px", backgroundColor: "#fff", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0, 0, 0, 0.1)" }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                        <span>
                                            <FilterTwoTone /><span style={{ fontWeight: 500 }}> Bộ lọc tìm kiếm</span>
                                        </span>
